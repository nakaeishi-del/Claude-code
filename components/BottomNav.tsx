      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white sm:hidden z-50"
        ...
        <div className="flex">
          {tabs.map((tab) => {
            ...
            const isTapped = tappedHref === tab.href
            return (
              <Link key={tab.href} href={tab.href}
                ...
                {active && (
                  <span className="absolute top-1 left-1/2 -translate-x-1/2 w-12 h-9 rounded-2xl"
                    style={{ background: '#FFF0EC', zIndex: -1 }} />
                )}

                {/* Pending vote badge */}
                {tab.href === '/dashboard' && pendingVotes > 0 && (
                  <span className="absolute top-1.5 right-1/2 translate-x-4 min-w-[16px] h-4 px-1 text-white text-[9px] font-black rounded-full flex items-center justify-center gentle-pulse"
                    style={{ background: '#F07050', zIndex: 1 }}>
